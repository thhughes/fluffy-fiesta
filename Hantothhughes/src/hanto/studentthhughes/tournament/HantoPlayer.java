...
public class HantoPlayer implements HantoGamePlayer
{
	...
	public HantoMoveRecord makeMove(HantoMoveRecord opponentsMove)
	{
		...
		if(result != null){
			makeAction(result, false);
		}
		theCounter.incrementNumberMoves(color);
		return result;
	}

	private HantoMoveRecord getOtherFirstMove() {
		Random r = new Random();
		int num = r.nextInt(firstMoveList.size());
		HantoMoveRecord result = firstMoveList.get(num);
		return result;
	}

	private void makeAction(HantoMoveRecord moveData, boolean isOpponent) {
		try {
			
			if(!isOpponent){
				if(moveData.getFrom() != null){
					theBoard.removeFromBoard(moveData.getFrom());
				}
				theBoard.placeOnBoard(new HantoPieceImpl(color,moveData.getPiece()),
						moveData.getTo());
				theCounter.incrementNumberMoves(color);
			}else{
				if(moveData.getFrom() != null){
				...
				theBoard.placeOnBoard(new HantoPieceImpl(getOpponentColor(),moveData.getPiece()),
						moveData.getTo());
				theCounter.incrementNumberMoves(getOpponentColor());
			}
