...
/**
 * This is an action finder class, it is used to locate actions that are valid for a 
 * specific game type.
 * @author Troy
 *
 */
public class ActionFinder {
	...
	/**
	 * constructor for the actionFinder class
	 * @param gameID
	 * 				HantoGameID of the game to be action-found
	 */
	...
	public HantoMoveRecord getAction(HantoBoard theBoard, MoveCounter theCounter, HantoPlayerColor color){
		...
		if(theCounter.getNumberMoves(color) > 4){
			Random r = new Random();
			int num = r.nextInt(2);
			if(num > 0){
				result = tryPlaceThenMoveAction(theBoard, theCounter, color);
			}else{
				result = tryMoveThenPlaceAction(theBoard, theCounter, color);
			}
		}else{
			result = tryPlaceThenMoveAction(theBoard, theCounter, color);
		}
	...
	private HantoMoveRecord tryPlaceThenMoveAction(HantoBoard theBoard, MoveCounter theCounter,
			HantoPlayerColor color) {
		HantoMoveRecord result;
		result = getPlaceAction(theBoard, theCounter, color);
		if(result == null){
	...
	private HantoMoveRecord tryMoveThenPlaceAction(HantoBoard theBoard, MoveCounter theCounter,
			HantoPlayerColor color) {
		HantoMoveRecord result;
